...
def hand_gesture():
    ...
    print('**********************************************************************')
    # Reading hsv values from the pickle file
    ...
    the previous cursor position (pyp). The new cursor position is then calculated accordingly
    ...
    # the user desires free movement of cursor, left click, right click or dragging (drag & drop)
    ...
    #  To capture a video, you need to create a VideoCapture object. open cv has a very simple interface for this,  argument is for no. of cameras
    ...
    while(1):
        #cv2. waitKey() is the function which bind our code with keyboard and any thing we type will be returned by this function.
        k = cv2.waitKey(10) & 0xFF
        # basically to record  P/C/R for caliberation options
        changeStatus(k)

        # Capture frame-by-frame
        _, frameinv = cap.read()
        # flip horizontaly to get mirror image in camera (1 means flipping around y axis)
        frame = cv2.flip( frameinv, 1)
        # frame = cv2.resize(frame,(cursor[0],cursor[1]))
...
print("Welcome to Virtual Mouse Aid\n")
z=True
while(z==True):
    print("Select the number according to the use\n")
    print("1-hand gesture\n")
    print("2-facial gesture\n")
    print("3-exit\n")
